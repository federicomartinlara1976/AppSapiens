...
public abstract class GenericDomainTableModel<T> implements TableModel {
	...
	/**
	 * Crea un nuevo {@code GenericDomainTableModel} sin datos con identificadores
	 * para las columnas.
	 *
	...
	public GenericDomainTableModel(@SuppressWarnings("rawtypes") List columnIdentifiers) {
		this();
	...
	/**
	 * Método conveniente para obtener obtener una lista con los TableModelListeners
	 ...
	 * ocurrido un evento. Nota: Los listeners son notificados en orden inverso al
	 * de suscripción.
	 */
	...
	protected void notifyTableRowsInserted(int firstRow, int lastRow) {
		TableModelEvent e = new TableModelEvent(this, firstRow, lastRow, TableModelEvent.ALL_COLUMNS,
	...
	/**
	 * Notifica que un objeto de dominio ha cambiado, causando una notificación en
	 * cascada hacia los listeners suscriptos.
	 * 
	 * @param domainObject El objeto de dominio
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Elimina una fila del table model según un índice. Nota: NO remueve todas las
	 * ocurrencias del objeto de dominio asociado al número de fila, sólo la
	...
	public void deleteRows(int firstRow, int lastRow) {
		if (firstRow < 0 || lastRow < 0 || firstRow > lastRow) {
			throw new IllegalArgumentException(
					"Los parámetros firstRow y lastRow deben ser positivos y firstRow >= lastRow.");
		} else {
